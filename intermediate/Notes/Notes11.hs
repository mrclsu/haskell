...
instance Foldable Tree where
  ...
  foldr f b (Node1 a t) = foldr f (f a b) t
  foldr f b (Node2 l r) = foldr f (foldr f b l) r

rightmost :: Tree a -> a
rightmost (Leaf a) = a
rightmost (Node1 _ t) = rightmost t
rightmost (Node2 _ r) = rightmost r

findElem :: (a -> Bool) -> Tree a -> Maybe a
findElem p (Leaf a) = if p a then Just a else Nothing
findElem p (Node1 a t) = if p a then Just a else findElem p t
findElem p (Node2 l r) = case findElem p l of
    Nothing -> findElem p r
    x -> x

instance Traversable Tree where
  traverse f (Leaf a) = Leaf <$> f a
  traverse f (Node1 a t) = Node1 <$> f a <*> (fmap f t)
  traverse f (Node2 l r) = Node2 <$> fmap f l <*> fmap f r
