...
data Tree a = Leaf a | Node1 a (Tree a) | Node2 (Tree a) (Tree a) deriving (Eq, Ord, Show)
...
instance Functor Tree where 
  ...
  fmap f (Node2 left right) = Node2 (fmap f left) (fmap f right)

instance Foldable Tree where -- elég az egyik
  foldr f b (Leaf a) = f a b 
  foldr f b (Node1 a t) = f a (foldr f b t) 
  foldr f b (Node2 left right) = foldr f (foldr f b right) left
...
findElem f (Leaf a) = case f a of 
  True -> Just a
  False -> Nothing
findElem f (Node1 a t) = case f a of
  True -> Just a
  False -> findElem f t
findElem f (Node2 left right) = case (findElem f left) of
  Nothing -> findElem f right
  Just a -> Just a

instance Traversable Tree where 
  traverse f (Leaf a) = Leaf <$> f a
  traverse f (Node1 a t) = Node1 <$> f a <*> traverse f t
  traverse f (Node2 left right) = Node2 <$> traverse f left <*> traverse f right

numberElems :: Tree a -> Tree (a, Int)  
numberElems t = evalState (go t) 0 where
  go a = do 
    n <- get
    case a of
      (Leaf b) -> put (n+1) >> pure (Leaf (b,n))
      (Node2 left right) ->  Node2 <$> go left <*> go right

replace :: [a] -> Tree a -> Tree a
replace l t = evalState (go t) l where
  go a = do
    l <- get
    case a of
      (Leaf b) -> case l of 
        [] -> pure (Leaf b)
        l1:l -> put l >> pure (Leaf l1)
      (Node2 left right) -> Node2 <$> go left <*> go right
